...
namespace SwitchManagment.API.SwitchService.Implementations
{
    public class SwitchServiceHPComware5 : ISwitchService
    {
        ...
        /*
        public async Task<SwitchInfo> GetSwitchInfoOld(ConnectConfig connectConfig, CancellationToken cancellationToken = default)
        {
        ...
        public async Task ConfigurePortOld(PortConfigTrunk portConfig, CancellationToken cancellationToken = default)
        {
            ...
            if (portConfig.TrunkVlans.Any(vlan => portConfig.TrunkVlans.Count(vl => vlan == vl) > 1))
            ...
            try
            {
                ...
                if(portConfig.TrunkVlans.Count() > 0)
                    shellStream.WriteLineAndExpect($"port trunk permit vlan {string.Join(' ', portConfig.TrunkVlans)}");
                #endregion
        ...
        public async Task ConfigurePortOld(PortConfigAccess portConfig, CancellationToken cancellationToken = default)
        {
            #region validation
            ArgumentNullException.ThrowIfNull(portConfig);
            ArgumentException.ThrowIfNullOrWhiteSpace(portConfig.IpOrName);
